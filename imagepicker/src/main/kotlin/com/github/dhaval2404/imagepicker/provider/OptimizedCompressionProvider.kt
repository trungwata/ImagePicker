 ...
 */
class OptimizedCompressionProvider(activity: ImagePickerActivity) : BaseProvider(activity) {

    companion object {
        private val TAG = OptimizedCompressionProvider::class.java.simpleName
    }
     ...
     */
    @WorkerThread
    private suspend fun startCompressionWorker(uris: List<Uri>) {
        try {
            val urisCompressed = uris.map { uriToCompress ->
                if (isCompressionRequired(uriToCompress)) {
                    FileUtil.getTempFile(this@OptimizedCompressionProvider, uriToCompress)?.let {
     ...
     */
    @WorkerThread
    private suspend fun startCompression(file: File): File? {
//        val newFile: File = Compressor.compress(activity, file) {
////            default(mMaxWidth, mMaxHeight, Bitmap.CompressFormat.JPEG)
//            resolution(mMaxWidth, mMaxHeight)
//            quality(80)
//            size(1_097_152)
//        }
//        return newFile

//        // Delete temp file after compress
//            file.delete()
